...
class CodeGenInterfaceNode(CodeGenImp):
    ...
    """
    def __init__(self, package_name: str, interface_name: str,function_name: str):
        super().__init__()
    ...
    def gen_code(self, file, element: Variable, root_var: Variable, depth: int, type_map: dict):
        ...
        file.content += self.indent(f"public interface {Variable.to_upper_camelcase(self.class_name)} {{", depth)
        file.content += self.indent(f"public LinkedList<TableRow> {Variable.to_variable(self.function_name)}({obj.type} {obj.get()}) throws Exception;", depth + 1)
        file.content += self.indent("}", depth)
...
class CodeGenClassNode(CodeGenImp):
    ...
    """
    def __init__(self, class_name: str, field_type: MessageFieldType):
        super().__init__()
    ...
    def gen_code(self, file, element: Variable, root_var: Variable, depth: int, type_map: dict):
        ...
        file.content += self.indent(f"public final class {Variable.to_upper_camelcase(self.class_name)} {{", depth)
